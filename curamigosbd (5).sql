...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 03-04-2022 a las 01:04:49
...
--
-- Estructura Stand-in para la vista `aceptados_c`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `aceptados_c` (
`id_emisor_fk` int(11)
);
...
--
-- Estructura Stand-in para la vista `enviados_c`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `enviados_c` (
`id_receptor_fk` int(11)
);
...
--
-- Estructura para la vista `aceptados_c`
--
DROP TABLE IF EXISTS `aceptados_c`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `aceptados_c`  AS SELECT `amigos`.`id_emisor_fk` AS `id_emisor_fk` FROM `amigos` WHERE `amigos`.`estado` = 'aceptado' AND `amigos`.`id_receptor_fk` = (select `personas`.`id` from `personas` where `personas`.`nombre` = 'carlos') ;
...
--
-- Estructura para la vista `enviados_c`
--
DROP TABLE IF EXISTS `enviados_c`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `enviados_c`  AS SELECT `amigos`.`id_receptor_fk` AS `id_receptor_fk` FROM `amigos` WHERE `amigos`.`estado` = 'aceptado' AND `amigos`.`id_emisor_fk` = (select `personas`.`id` from `personas` where `personas`.`nombre` = 'carlos') ;
